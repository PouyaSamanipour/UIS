...
def plotting_results_BF(TH,all_hyperplanes,all_bias,c_v,W_v,W,c,enumerate_poly,name):
    ...
    plt.contour(X1, X2, Z,[0],colors='red')
    ...
    red_line = mlines.Line2D([], [], color='red', marker="_",label="Invariant sets",
                          markersize=15) 
...
def plot_polytope(enumerate_poly, color):
    for i in enumerate_poly:
        hull = ConvexHull(i)
        for simplex in hull.simplices:
            plt.plot(i[simplex, 0], i[simplex, 1], color)
        # plt.plot(i[hull.vertices,0], i[hull.vertices,1], 'r--', lw=2)
        # plt.plot(i[hull.vertices[0],0], i[hull.vertices[0],1], 'ro')
    plt.xlabel('X')
    plt.ylabel('Y')
...
def plot_polytope_2D(NN_model,TH):
    ...
    with torch.no_grad():  # No need to track gradients for evaluation
        output = model(torch.FloatTensor(data))
    plt.streamplot(X1,X2,np.array(output[:,0]).reshape(len(X1),len(X2)),np.array(output[:,1]).reshape(len(X1),len(X2)),color=('cyan'))
    # plt.show()
def plot_invariant_set(model,TH):
    # import torch
    # model = torch.jit.load(NN_model)
...
def plot_level_set(model,TH):
    ...
    Z=np.asarray(output)
    Z=np.reshape(Z,(len(x1),len(x2)))
    plt.contour(X1, X2,Z,10,colors='red')
    plt.xlabel('X')
    plt.ylabel('Y')
    # plt.show()