...
def Finding_Lyapunov_function(NN_file,name_figure,eps1,eps2,TH,mode,parallel):
    ...
    while status:
        ...
        if not status:
            ...
            a=1.0
            NN_file,_,_,_,_=updating_NN(NN_file,n,all_hyperplanes,all_bias,W_v,c_v)
...
def updating_NN(NN_file,n,hype,b,W_v,c_v):
    ...
    if status:
        ...
        NN_file=NN_file[:-3]+"_Lyap_updated.pt"
        torch.jit.save(torch.jit.script(MGPU), NN_file)
