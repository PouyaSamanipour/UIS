...
def preprocessing_BF(enumerate_poly,D,W,c,hyperplanes,b,eps1,eps2,TH,alpha):
    ...
    boundary_vertices=[]
    # boundary_regions=[]
    boundary_regions_org,index_list,V,Xdot,states=finding_boundary_Regions(enumerate_poly,TH,hyperplanes,b,c,W)
    ...
    #     print(constr)
    #print(sol)
    return sol,n_h,n_r,n,boundary_vertices,boundary_regions_org
    ...
    for k, i in enumerate(enumerate_poly):
        ...
        for j in range(len(i)):
            ...
            states.append(state)    
                ...
                #     # if j@xdot>1e-8:
                #         regions.append(k)  # Append k to the list
    return np.unique(np.array(regions)),index_list,V,Xdot,states  # Convert to array and return unique values
